...
@router.get("/ping", name="health checks", description="returns 200")
...
@router.get(
    "/auth-ping", name="authorized ping", description="returns 200 if authorized"
)
async def auth_ping(
    token_payload: dict = Depends(auth_service.validate_token),
) -> JSONResponse:
