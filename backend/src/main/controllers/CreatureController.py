...
async def population_positions_example(population_size: int = 10, gameboard_size: GameboardSize = GameboardSize.XL,
    ...
    return evolution_service.get_population_positions_example(population_size, gameboard_size, lifecycle_steps)
...
async def get_population_positions_encoded() -> StreamingResponse:
    ...
    creatures_data = evolution_service.get_population_positions_example(1000, GameboardSize.XL, 300)
    data_bytes = evolution_service.encode_positions_to_bytes(300, creatures_data)
    response = StreamingResponse(iter([data_bytes]), media_type="application/octet-stream")
